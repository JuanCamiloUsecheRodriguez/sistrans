...
public class DAOTablaFunciones {
	...
	public ArrayList<Funcion> darFuncionesDeCompa�iaDeTeatro(String nombreCompa�ia) throws SQLException
	{
		ArrayList<Funcion> funciones = new ArrayList<Funcion>();

		String sql = "SELECT * FROM (SELECT * FROM FUNCION FULL OUTER JOIN ESPECTACULO ON FUNCION.ESPECTACULO = ESPECTACULO.ID)T1 FULL OUTER (SELECT * FROM PATROCINA FULL OUTER JOIN COMPANIA ON PATROCINA.IDCOMPANIA = COMPANIA.ID)T2 ON T1.ID = T2.IDESPECTACULO WHERE T2.NOMBRE = " + nombreCompa�ia;
