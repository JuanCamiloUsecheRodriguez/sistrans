...
public class JMSRentabilidadCompania implements MessageListener, ExceptionListener
{
	 ...
	 */
	private String topicAllReporteCompania;
	 ...
	 */
	public final static String GET_ALL_VIDEO_ASK = "GETALLReporteCompania";

	/**
	 * Atributo que representa, dentro del mensaje, la respuesta del requerimiento dar todos los videos.
	 */
	public final static String GET_ALL_VIDEO_REPLY = "GETALLReporteCompaniaRES";
	 ...
	 */
	public static JMSRentabilidadCompania darInstacia(FestivAndesMaster festivAndesMaster){
		instancia = instancia == null? new JMSRentabilidadCompania() : instancia;
	...
	public void cerrarRecursos() {
		for(Object ob : recursos){
			if(ob instanceof DAOTablaClientes)
				try {
					((DAOTablaClientes) ob).cerrarRecursos();
				} catch (Exception ex) {
					ex.printStackTrace();
				}
		}
	 ...
	 */
	public void setUpJMSManager(int numerApps, String myQueue, String topicAllReporteCompania){
		try {
			this.numberAppsTotal = numerApps - 1;
			this.myQueue = myQueue;
			this.topicAllReporteCompania = topicAllReporteCompania;
			setupMyQueue();
			setupSubscriptions();
			waiting = false;
			respuesta = new ListaReporteCompania();
	 ...
	 */
	public ListaReporteCompania getReporteCompaniaResponse() throws IncompleteReplyException, JMSException, NamingException, InterruptedException, NonReplyException {
		...
		if(count == TIME_OUT){ // Verifica si se cumplió el time out 
			if(this.respuesta.getReportes().isEmpty()){
				waiting = false;
				this.numberApps = 0;
		...
		waiting = false;
		this.numberApps = 0;
		if(respuesta.getReportes().isEmpty())
		...
		ListaReporteCompania res = respuesta;
		respuesta = new ListaReporteCompania();
	...
	public void setupSubscriptions()
	{
		...
		try {
			...
			this.topic = (Topic) ctx.lookup(topicAllReporteCompania);
	...
	public void onMessage(Message message)
	{
		...
		try 
		{
			...
			if(a[0].equals(GET_ALL_VIDEO_ASK) && !a[1].equals(this.myQueue)){
				ListaReporteCompania ReporteCompania = this.master.darReporteCompaniaLocal();
				ObjectMapper mapper = new ObjectMapper();
				String jsonString = mapper.writeValueAsString(ReporteCompania);
				doResponseToQueue(a[1], GET_ALL_VIDEO_REPLY + CONNECTOR + jsonString);
			}
			else if(a[0].equals(GET_ALL_VIDEO_REPLY)){
				ObjectMapper mapper = new ObjectMapper();
				if(waiting){
					ListaReporteCompania obj = mapper.readValue(a[1], ListaReporteCompania.class);
					this.respuesta.addReporteCompania(obj);
					this.numberApps++;
				}
			}
